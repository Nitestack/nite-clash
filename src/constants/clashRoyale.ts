class ClashRoyaleElixir {
  ...
  public static getElixirCosts(card: string) {
    ...
    for (let i = 1; i < 10; i++) {
      const cards: string[] =
        //@ts-ignore: dynamically get the cards
        ClashRoyaleElixir[`${ClashRoyaleElixir.numbers[i]}ElixirCards`];
      if (cards.includes(card)) {
        elixirCosts = i;
        break;
      }
    }
    if (ClashRoyaleElixir.onePointFiveElixirCards.includes(card))
      elixirCosts = 1.5;
    return elixirCosts;
  }
  ...
  private static threeElixirCards = [
    "Knight",
    "Archers",
    "Minions",
    "Skeleton Army",
  ...
  private static fourElixirCards = [
    "Valkyrie",
    "Musketeer",
    "Baby Dragon",
    "Mini P.E.K.K.A",
  ...
  private static fiveElixirCards = [
    "Giant",
    "Balloon",
    "Witch",
    "Barbarians",
  ...
  private static sevenElixirCards = [
    "P.E.K.K.A",
    "Lava Hound",
    "Royal Recruits",
    "Mega Knight",
...
export default class CrConstants {
  ...
  public static openProfile(tag: string, isMobile?: boolean) {
    return `${
      isMobile ? "clashroyale://" : "https://link.clashroyale.com/en?"
    }playerInfo?id=${tag.toUpperCase().replace(/#/g, "")}`;
  }

  public static openClan(tag: string, isMobile?: boolean) {
    return `${
      isMobile ? "clashroyale://" : "https://link.clashroyale.com/en?"
    }clanInfo?id=${tag.toUpperCase().replace(/#/g, "")}`;
  }
}
